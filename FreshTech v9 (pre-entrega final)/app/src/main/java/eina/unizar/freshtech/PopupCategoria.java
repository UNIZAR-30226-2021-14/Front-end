...
public class PopupCategoria extends AppCompatDialogFragment implements Callback<Registrar> {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        if(extras != null) {
            nombreEditar = extras.getString("nombreEditar");
            if(nombreEditar != null) {
                editTextNombreCategoria.setText(nombreEditar);
                editTextNombreCategoria.setSelection(editTextNombreCategoria.getText().length());
            }
        }
        if (titulo.equals("Añadir")) {
                    ...
                    public void onClick(View view) {
                        ...
                        if (!nombreCategoria.isEmpty()) {
                            if(nombreEditar == null) {
                                ...
                                String token = "Bearer " + Preferencias.cargarToken(getActivity());
                                Call<Registrar> callEditarCategoria = ApiAdapter.getApiService().editarCategoria(token, nombreEditar, nombreCategoria);
                                callEditarCategoria.enqueue(new Callback<Registrar>() {
                                    @Override
                                    public void onResponse(Call<Registrar> call, Response<Registrar> response) {
                                        if(response.isSuccessful()) {
                                            ...
                                            if(registrar.getMessage().equals("ok")) {
                                                ...
                                                if(getActivity() instanceof ImagenEdit) {
                                                    mOnCategoriaCreadaEdit.recargarSpinner(editTextNombreCategoria.getText().toString());
                                                }
                                                if(getActivity() instanceof DocumentoEdit) {
                                                    mOnCategoriaCreadaEdit.recargarSpinner(editTextNombreCategoria.getText().toString());
                                                }
                                                ...
                                                Toast.makeText(getActivity(), "Ha ocurrido un error al editar la categoría", Toast.LENGTH_SHORT).show();
                                            }
                                        }
                                        else {
                                            Toast.makeText(getActivity(), "Ha ocurrido un error al editar la categoría", Toast.LENGTH_SHORT).show();
    ...
    public void onAttach(@NonNull Context context) {
        ...
        try {
            ...
            } else if(getActivity() instanceof ImagenEdit) {
                mOnCategoriaCreadaEdit = (OnCategoriaCreadaEdit) getActivity();
            } else if(getActivity() instanceof DocumentoEdit) {
                mOnCategoriaCreadaEdit = (OnCategoriaCreadaEdit) getActivity();
            } else {
